...
import com.rytmo.library.services.SolanaService.Companion.toUsdcLamports
...
class SendResource {
    ...
    fun sendSolana(request: SendSolanaRequest, @Context requestContext: ContainerRequestContext): Response {
        ...
        return try {
            ...
            val txBase64 =
                solanaService.buildUsdcTransferTransaction(
                    fromAddress = wallet.address,
                    toAddress = request.recipientAddress,
                    lamports = request.amountUsdc.toUsdcLamports(),
                )
            val signature = privyServerWalletService.signAndSend(wallet.walletId, txBase64)
